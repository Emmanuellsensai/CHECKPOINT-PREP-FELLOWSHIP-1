...
isOp(op)               -> true if op is one of the five valid operations
applyOp(word, op)      -> returns the transformed word for a given operation
checkPunctuations(text)-> no space before punctuation, one space after; fix quotes
checkVowels(text)      -> "a apple" -> "an apple"
```

`isOp` and `applyOp` are small helpers pulled out of `checkFlags` so the main loop stays short and readable. That split is your answer when they ask "why these extra functions?": each does one job.
...
This version builds a clean `result` list as it goes. Normal words get added. When it hits a marker, it reaches back into `result` and transforms the last `count` words, then skips the marker by simply not adding it. No deletion line, no `i--`.

```go
func checkFlags(text string) string {
	words := strings.Fields(text) // split on whitespace into a slice of words
	var result []string           // the clean list we build up

	for i := 0; i < len(words); i++ {
		word := words[i]
		op := ""
		count := 1 // how many previous words to change (default 1)
		skip := 0  // how many extra tokens to jump (1 for the complex form)
		...
		if strings.HasPrefix(word, "(") && strings.HasSuffix(word, ")") {
			...
			i+1 < len(words) && strings.HasSuffix(words[i+1], ")") {
			op = strings.Trim(word, "(,") // "(up," -> "up"
			if n, err := strconv.Atoi(strings.Trim(words[i+1], ")")); err == nil {
				count = n
				skip = 1 // also jump the "2)" token
			}
		}

		if isOp(op) {
			// Reach BACKWARD into result: j=1 is the last word added, j=2 the one before.
			for j := 1; j <= count && len(result)-j >= 0; j++ {
				result[len(result)-j] = applyOp(result[len(result)-j], op)
			}
			i += skip // jump the count token if it was the complex form
			continue  // skip the marker by NOT adding it to result
		}

		result = append(result, word) // a normal word, keep it
	}
	return strings.Join(result, " ")
}
```

The two helpers, kept tiny so the main loop reads cleanly:

```go
func isOp(op string) bool {
	return op == "hex" || op == "bin" || op == "up" || op == "low" || op == "cap"
}

func applyOp(word, op string) string {
	switch op {
	case "up":
	...
	case "cap":
		return strings.Title(strings.ToLower(word)) // first letter up
	case "hex":
		val, _ := strconv.ParseInt(word, 16, 64) // read as base 16
		return fmt.Sprint(val)                    // number -> string
	case "bin":
		val, _ := strconv.ParseInt(word, 2, 64) // read as base 2
...
1. **Two shapes of marker.** Simple `(up)` is one word (starts `(`, ends `)`). Complex `(up, 2)` is two words (`(up,` then `2)`). The `if / else if` detects which, and sets `op`, `count`, and `skip`.
2. **Build, do not delete.** Instead of removing the marker from the list you are walking, you build a fresh `result` and only append normal words. The marker is "removed" just by never adding it. That is why there is no `append(result[:i], ...)` line and no `i--`.
3. **Reaching backward into `result`.** `result[len(result)-j]` is the j-th word from the end of what you have collected so far. `j` runs from 1 up to `count`, and `len(result)-j >= 0` stops it walking off the front. That loop IS the "affect the previous word(s)" rule.

**Say it out loud:** "I build `result` as I go. Normal words get appended. When I hit a marker, I reach back into `result` and transform the last `count` words with `result[len(result)-j]`, then skip the marker by not appending it. `i += skip` jumps the count token for the complex form."

**Trace `so cool (up, 2)`:** words = `["so","cool","(up,","2)"]`.
- `i=0`: "so" is normal, `result = ["so"]`.
- `i=1`: "cool" is normal, `result = ["so","cool"]`.
- `i=2`: "(up," is the complex form. `op="up"`, `count=2`, `skip=1`. Reach back: `j=1` turns `result[1]` "cool" into "COOL"; `j=2` turns `result[0]` "so" into "SO". `i += 1` jumps the "2)" token. Marker not appended.
- Result joined: `SO COOL`.
...
1. **`strings.Title`** is officially deprecated (still works for plain ASCII). If asked: "I know it's deprecated, but I lowercase first so it only capitalizes the first letter, and it works fine for ASCII."
2. **`fmt.Sprint(val)`** just turns the number into a string, same result as `strconv.FormatInt(val, 10)`.
3. **`skip`** exists only to handle the complex form. It is 0 for a simple marker and 1 for `(up, 2)`, so `i += skip` jumps past the `2)` token only when there is one. The bounds check `i+1 < len(words)` in the complex branch means it can never read past the end.

## Tricky bits checklist

- A marker affects the words **before** it, reached with `result[len(result)-j]`.
- `(up, 2)` is **two tokens** after `strings.Fields`: `(up,` and `2)`.
- The marker is "removed" by **never appending it** to `result`, not by deleting from a list.
- `i += skip` jumps the `2)` token only for the complex form (`skip` is 1 there, 0 otherwise).
...
2. What is the difference between the "simple" and "complex" marker, and how does the `if / else if` tell them apart?
3. How does the marker get removed from the output without any deletion line? What replaced the old `i--`?
4. What does `isOp` do, and why pull it and `applyOp` out of the main loop?
5. In `checkPunctuations`, what are the two rules in the first loop, and how do groups like `...` stay together?
6. Walk through `checkVowels` on `a apple` and on `a hour`.

## Practice - WRITE IT (blank file, no peeking)

1. Write the `for j := 1; j <= count && len(result)-j >= 0; j++` loop and explain `result[len(result)-j]`.
2. Write `applyOp` from memory, including the hex and bin cases.
